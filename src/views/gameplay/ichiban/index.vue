<template>
  <div class="page ichiban-page">
    <el-card>
      <template #header>
        <div style="display: flex; align-items: center; justify-content: space-between">
          <span>一番赏（活动类型：赏池）</span>
          <el-button type="primary" @click="handleAdd">新增赏池</el-button>
        </div>
      </template>
      <el-form :inline="true" class="filter-form">
        <el-form-item label="赏池名称">
      ...
      <el-table :data="list" stripe style="width: 100%" v-loading="loading">
        <el-table-column prop="title" label="赏池名称" min-width="160" />
        ...
        <el-table-column label="操作" width="150" fixed="right">
          <template #default="{ row }">
            ...
            <el-popconfirm title="确定删除该赏池？" @confirm="handleDelete(row.id)">
              <template #reference>
    ...
    <el-dialog v-model="dialogVisible" :title="isEdit ? '编辑赏池' : '新增赏池'" width="720px" :close-on-click-modal="false">
      <el-form ref="formRef" :model="form" :rules="formRules" label-width="120px">
        <el-form-item label="赏池名称" prop="title">
...
const formRules: FormRules = {
  title: [{ required: true, message: '请输入赏池名称', trigger: 'blur' }],
...
async function fetchData() {
  loading.value = true
  try {
    const params: Record<string, unknown> = { page: query.page, size: query.size, activityType: ACT_TYPE_ICHIBAN }
...
function goToBoxes(row: ActivityVO) {
  router.push({ path: `/gameplay/ichiban/${row.id}/boxes` })
}
